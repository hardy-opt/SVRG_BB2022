...
% Modified by H.Kasai on Mar. 23, 2018
    ...
    methods
        ...
        function g = grad(obj, w, ind,last)

            alpha = w' * obj.x_train(:,ind:last);
            flag = obj.y_train(ind:last) .* alpha;
            flag(flag<1.0) = 1;
            flag(flag>1.0) = 0;

            coeff = flag' .* (1 - obj.y_train(ind:last)' .* alpha') .* obj.y_train(ind:last)';
            coeff = coeff';
            coeff = repmat(coeff,[obj.d 1]);
            g = obj.lambda * w - sum(coeff .* obj.x_train(:,ind:last),2)/length(ind:last);  
        ...
        function h = hess(obj, w, ind,last)

            alpha = w' * obj.x_train(:,ind:last);
            flag = obj.y_train(ind:last) .* alpha;
            flag_indices = flag<1.0;

            x_part_new = obj.x_train(:,flag_indices);
            x_part_new = x_part_new(:,flag_indices);

            h = obj.lambda * eye(obj.d) + x_part_new * x_part_new'/length(ind:last); 
        end
